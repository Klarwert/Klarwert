...
, direction text not null default 'ausgabe' check (direction in ('ausgabe', 'einnahme')) -- nur an oberkategorien
                                                                                          -- gepflegt, unterkategorien
                                                                                          -- übernehmen die des parents;
                                                                                          -- steuert nur sortierung/dimming
                                                                                          -- in der combobox, keine harte filterung
, parent_id integer references categories(id) on delete restrict
, template_key text unique -- stabiler slug für template-kategorien (z.b. 'wohnen', 'wohnen.strom');
                           -- NULL bei eigenen kategorien. ermöglicht idempotentes re-seeding beim start,
                           -- ohne umbenennungen/ausblendungen des nutzers zu überschreiben (siehe CLAUDE.md)
...
-- ebene a: kuratierte globale händler-datenbank (siehe klarwert-community-haendler-db.md).
-- getrennt vom code als daten-release versioniert; via community-quelle aktualisierbar.
create table merchants (
  id integer primary key
, canonical_name text not null unique -- z.b. 'rewe', 'netflix' (normalisiert, lowercase)
, display_name text not null -- z.b. 'REWE' (anzeige)
, default_category_id integer references categories(id) on delete set null
, source_version text -- daten-release, z.b. '2026-07'; NULL bei lokal ergänzten händlern
, is_builtin integer not null default 0 -- aus kuratierter quelle vs. lokal ergänzt
, is_active integer not null default 1
);
...
, match_value text not null -- normalisierter vergleichswert oder muster
, priority integer not null default 100 -- niedriger = zuerst geprüft (iban vor name_exact vor fuzzy)
);
create index idx_merchant_aliases_value on merchant_aliases(match_value);

-- persönliche unterdrückung einer globalen händler-zuordnung (statt löschung – bleibt für andere intakt)
create table merchant_suppressions (
  id integer primary key
, merchant_id integer not null references merchants(id) on delete cascade
, suppressed_at text not null default (datetime('now'))
);

-- benutzerdefinierte spalten für transaktionen (jetzt mit datentyp statt reinem text)
create table custom_fields (
  id integer primary key
, name text not null unique
, data_type text not null default 'text' check (data_type in ('text', 'integer', 'decimal', 'boolean', 'date', 'datetime'))
, sort_order integer not null default 0
, is_deleted integer not null default 0
);
...
, value text not null -- immer als text gespeichert, gemäß custom_fields.data_type interpretiert/formatiert
...
, categorization_source text not null default 'none' check (categorization_source in
    ('none', 'manual', 'rule', 'contract', 'merchant', 'similarity'))
, applied_rule_id integer references rules(id) on delete set null
, merchant_id integer references merchants(id) on delete set null -- erkannter händler, falls vorhanden
, categorization_confidence real -- 0-1, für transparenz-anzeige (siehe product spec)
...
, created_from text not null default 'manual' check (created_from in ('manual', 'aufraeumen', 'vertrag'))
                                     -- herkunft, wird in der regel-liste angezeigt ("automatisch aus vertrag x erstellt")
, source_contract_id integer references contracts(id) on delete set null -- nur gesetzt, wenn created_from='vertrag'
, created_at text not null default (datetime('now'))
, is_deleted integer not null default 0
...
, category_id integer references categories(id) on delete set null
, merchant_id integer references merchants(id) on delete set null -- optionaler händler-anker (siehe unten)
...
, confidence real -- 0-1, aus wiederholungsanzahl + betragsvarianz + intervall-regelmäßigkeit
, detected_at text not null default (datetime('now'))
, is_dismissed integer not null default 0 -- getrennt: muster wird nicht erneut vorgeschlagen
, is_deleted integer not null default 0
);
-- hinweis: die beim ersten bestätigen erzeugte regel wird über rules.source_contract_id rückverknüpft,
-- NICHT über eine spalte contracts.generated_rule_id. das vermeidet eine zirkuläre foreign-key-abhängigkeit
-- zwischen contracts und rules, die in sqlite bei table-rebuild-migrationen zum fehler
-- "no such table: main.contracts_old" führt.

create table recurring_payments (
  id integer primary key
, name text not null -- generiert, umbenennbar
, typical_amount_cents integer not null -- gleitender durchschnitt
, category_id integer references categories(id) on delete set null -- optional, jetzt editierbar (vorher gar nicht vorhanden)
, detected_at text not null default (datetime('now'))
...
-- protokoll je transaktion: welche stufe/regel hat die kategorie gesetzt (für transparenz-anzeige,
-- getrennt vom undo-orientierten history_log)
...
create index idx_categorization_log_tx on categorization_log(transaction_id);
