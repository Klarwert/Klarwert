...
export type MerchantAliasMatchType = "account_identifier" | "iban" | "name_exact" | "name_fuzzy" | "regex";
...
export interface Merchant {
  ...
  is_active: 0 | 1;
  is_modified: 0 | 1;
  /** @deprecated Nie wirklich genutzt; bleibt im Schema für Kompatibilität erhalten. */
  country?: string;
  /**
   * Herkunft des Eintrags (Migration 031): 'system' (seedDefaultMerchants), 'community'
   * (applyMerchantDataRelease) oder 'user' (manuell angelegt/verändert). Rein informativ/auditierbar -
   * die tatsächliche Konfliktlogik (Community darf lokale Änderungen nie überschreiben) läuft
   * weiterhin über `is_modified`, nicht über dieses Feld.
   */
...
export interface MerchantAlias {
  ...
  match_field: "counterparty" | "purpose" | "any";
  match_value: string;
  priority: number;
}
...
export interface Asset {
  ...
  iban: string | null; // @deprecated: use account_identifier instead
  account_identifier: string | null;
  account_identifier_type: "iban" | "us_routing_account" | "other";
  currency: string;
  is_archived: 0 | 1;
  is_deleted: 0 | 1;
  created_at: string;
...
export type ContractStatus = "detected" | "confirmed" | "price_changed" | "paused" | "ended" | "suggested_ended";
...
export type { SettingsMap } from "@/lib/settings/registry";
