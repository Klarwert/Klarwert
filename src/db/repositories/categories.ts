...
/** Löst einen stabilen Template-Key (siehe TEMPLATE_CATEGORIES) zu seiner lokalen id auf. */
export async function getCategoryIdByTemplateKey(templateKey: string): Promise<number | null> {
  const db = await getDb();
  const rows = await db.select<{ id: number }[]>(
    "select id from categories where template_key = $1 and is_deleted = 0",
    [templateKey],
  );
...
export async function createCategory(input: {
  ...
  await logOperation(db, "insert", "categories", id, { ...input, is_template: 0, sort_order: 999 }, null);
  return id;
}
...
export async function updateCategory(
  ...
  const oldRows = await db.select<Category[]>("select * from categories where id = $1", [id]);
  ...
  await logOperation(db, "update", "categories", id, input, oldRow);
}
...
export async function deleteCategory(id: number): Promise<void> {
  ...
  await logOperation(db, "delete", "categories", id, {}, oldRows[0]);
}
