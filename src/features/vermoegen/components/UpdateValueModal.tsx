  ...
  return (
    <Dialog open={!!asset} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[400px]">
        <DialogHeader>
          <DialogTitle>{t("updateValue.title", { name: asset.name })}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label htmlFor="update-value">{t("updateValue.amount")}</Label>
            <AmountInput
              id="update-value"
              placeholder="0,00"
              value={value}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="update-value-date">{t("updateValue.date")}</Label>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => void handleSubmit()} disabled={submitting || !value.trim()}>
            {t("updateValue.save")}
          </Button>
        </DialogFooter>
      </DialogContent>
