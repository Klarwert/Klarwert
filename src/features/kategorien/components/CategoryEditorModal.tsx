  ...
  useEffect(() => {
    ...
    if (category) {
      ...
      if (category.is_template === 0 && category.is_system === 0) {
        void getCategoryAliases(category.id).then(setAliases);
      } else {
        setAliases([]);
      }
    } else {
      ...
      <DialogContent className="max-w-[480px]">
            ...
            <>
              ...
              {IconPreview && (
                <div className="flex items-center gap-2 text-sm text-slate">
                  {t("rules.preview", { count: ""}).replace(" ( Treffer)", "").replace(" ()", "")}: <IconPreview className="size-4" style={{ color }} /> {name || t("categories.addBtn")}
                </div>
              )}
            </>
          )}
        ...
        {isOwnCategory && (
          <div className="space-y-2">
            <Label>{t("categories.searchAliases")}</Label>
            ...
            <div className="flex gap-2">
              <Input
                value={newAlias}
                placeholder={`${t("merchants.addAlias")}...`}
                maxLength={40}
              ...
              <Button
                ...
                size="icon"
                variant="outline"
                aria-label={t("merchants.addAlias")}
