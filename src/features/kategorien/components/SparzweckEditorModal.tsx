...
export function SparzweckEditorModal({ open, sparzweck, onOpenChange, onSaved }: SparzweckEditorModalProps) {
  const { t } = useTranslation(["vermoegen", "app", "kategorien"]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[400px]">
        <DialogHeader>
          <DialogTitle>{sparzweck ? t("sparzwecke.edit") : t("sparzwecke.add")}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="sz-name">{t("sparzwecke.name")}</Label>
            <Input id="sz-name" value={name} maxLength={40} onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label>{t("kategorien:categories.color")}</Label>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="sz-target">{t("sparzwecke.targetAmount")} ({t("vermoegen:create.defaultSavingsGoal").includes("optional") ? "optional" : "optional"})</Label>
