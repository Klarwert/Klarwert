        ...
        <SheetContent className="w-[430px] overflow-y-auto sm:max-w-[430px]">
          ...
          <div className="mt-4 space-y-4 text-sm">
            <div className="flex justify-between">
              <span className="text-slate">{t("categories.yearSum")}</span>
              <span className="num text-charcoal">{formatEur(yearSums?.get(category.id) ?? 0)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate">{t("categories.transactions")}</span>
              <span className="num text-charcoal">{usage ?? 0}</span>
            </div>

            <div>
              <div className="mb-2 flex items-center justify-between">
                <span className="font-medium text-charcoal">{t("rules.title")}</span>
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => {
              ...
              <div className="space-y-2">
                {rules?.map((rule) => (
                  ...
                  >
                    {rule.groups
                      .map((g) => g.conditions.map((c) => {
                        const fieldKey = "rules.condition.field." + c.field;
                        const opKey = "rules.condition.operator." + c.operator;
                        return `${t(fieldKey, c.field)} ${t(opKey, c.operator)} "${c.value}"`;
                      }).join(` ${t("rules.and")} `))
                      .join(` ${t("rules.or")} `)}
                  </button>
                ))}
                {(!rules || rules.length === 0) && (
                  <p className="text-xs text-slate">{t("rules.noRules")}</p>
                )}
              </div>
            </div>
          </div>
