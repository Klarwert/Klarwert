...
const EXTRA_FIELD_LABELS: Record<string, string> = {
  ...
  card_payment_at: "card_payment_at",
  cash_withdrawal_at: "cash_withdrawal_at",
...
export function TransactionDrawer({ transaction, onOpenChange, onSaved }: TransactionDrawerProps) {
  ...
  useEffect(() => {
    if (transaction) {
      ...
      void getTransactionCollectionIds(transaction.id).then(setCollectionIds);
    }
  }, [transaction]);
  ...
  function ruleLabel(id: number | null | undefined): string {
    ...
    if (!condition) return t("drawer.stage.manual");
    const fieldKey = RULE_FIELD_LABELS[condition.field] ?? condition.field;
    const operatorKey = RULE_OPERATOR_LABELS[condition.operator] ?? condition.operator;
    const fieldLabel = t(`kategorien:rules.condition.field.${fieldKey}`, fieldKey);
    const operatorLabel = t(`kategorien:rules.condition.operator.${operatorKey}`, operatorKey);
    return `${t("drawer.categorization.rule")}: ${fieldLabel} ${operatorLabel} „${condition.value}"`;
  }
  ...
  function stageLabel(matchedBy: CategorizationMatchedBy, ruleId: number | null, merchantId: number | null): string {
    ...
    if (matchedBy === "merchant_iban") return `${t("drawer.stage.merchant_db")} · ${t("drawer.stage.iban_match")}: ${merchantLabel(merchantId)}`;
    if (matchedBy === "merchant_alias") return `${t("drawer.stage.merchant_db")} · ${t("drawer.stage.alias_match")}: ${merchantLabel(merchantId)}`;
    return t(`drawer.stage.${matchedBy}`, { defaultValue: t("drawer.stage.none") });
  }
  ...
  function alternativeLabel(alt: CategorizationAlternative): string {
    ...
    if (alt.matched_by === "similarity") {
      return `${t("drawer.stage.similarity")} → ${categoryLabel(alt.category_id)} (${confidencePercent} % ${t("drawer.categorization.confidence")})`;
    }
    return `${merchantLabel(alt.merchant_id)} → ${categoryLabel(alt.category_id)} (${confidencePercent} % ${t("drawer.categorization.confidence")})`;
  }
        ...
        <div className="mt-4 space-y-5">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label htmlFor="tx-date">{t("drawer.bookingDate")}</Label>
                  <DateInput id="tx-date" max={todayIso()} value={bookingDate} onChange={setBookingDate} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="tx-amount">{t("drawer.amount")}</Label>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="tx-counterparty">{t("drawer.counterparty")}</Label>
                <Input id="tx-counterparty" value={counterparty} onChange={(e) => setCounterparty(e.target.value)} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="tx-purpose">{t("drawer.purpose")}</Label>
                ...
                <p className="text-xs text-slate">
                  {categorizationLog
                    ...
                    : transaction.categorization_source === "manual"
                      ? t("drawer.stage.manual")
                      : t("drawer.stage.none")}
                </p>
                {categorizationLog && (
                  <Badge variant="outline" className="shrink-0 text-[10px]">
                    {Math.round(categorizationLog.confidence * 100)} % {t("drawer.categorization.confidence")}
                  </Badge>
                )}
              </div>
              {logAlternatives.length > 0 && (
                <Collapsible className="mt-1.5">
                  <CollapsibleTrigger className="flex items-center gap-1 text-xs text-slate underline underline-offset-2">
                    ...
                    {t("drawer.categorization.alternatives")} ({logAlternatives.length})
                  </CollapsibleTrigger>
          ...
          <div className="space-y-1.5">
            ...
            {tags && tags.length > 0 ? (
              ...
              <p className="text-xs text-slate">{t("app:common.none")}</p>
            )}
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="tx-saving">{t("drawer.saving")}</Label>
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="tx-reviewed">{t("drawer.reviewed")}</Label>
            <Switch id="tx-reviewed" checked={isReviewed} onCheckedChange={setIsReviewed} />
          </div>
          <div className="flex items-center justify-between">
            <Label htmlFor="tx-transfer">{t("drawer.transfer")}</Label>
            <Switch
              id="tx-transfer"
              checked={isTransfer}
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="tx-exclude">{t("drawer.excludeFromStats")}</Label>
            <Switch id="tx-exclude" checked={excludeFromStats} onCheckedChange={setExcludeFromStats} />
          </div>

          {transaction.is_transfer === 1 && (
            <Badge className="bg-sage text-card hover:bg-sage">{t("status.transfer")}</Badge>
          )}

          {customFields && customFields.length > 0 && (
            <div className="pt-2">
              <Label className="mb-2 block text-sm">{t("drawer.customFields")}</Label>
              <div className="space-y-3">
          ...
          {extraFieldEntries.length > 0 && (
            <Collapsible>
              <CollapsibleTrigger className="flex items-center gap-1 text-sm text-petrol">
                ...
                {t("app:common.more")} {t("drawer.moreBankData")}
              </CollapsibleTrigger>
          ...
          <div className="pt-2">
            <Label className="mb-2 block text-sm">{t("drawer.collections")}</Label>
          ...
          <div className="pt-2">
            ...
            >
              {t("drawer.categorization.rule")} {t("app:common.create").toLowerCase()}
            </Button>
          </div>
        </div>
