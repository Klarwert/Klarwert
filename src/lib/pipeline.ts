...
// Architektur-Entscheidung: Zwei Händler-Lookup-Pfade sind bewusst getrennt (ADR-intern).
// - pipeline.ts (findMerchantMatch): vollständiger Multi-Pass-Match (IBAN → Alias → Fuzzy-Similarity) für den
//   Import-Zeitpunkt. Kann logCategorization schreiben und Benachrichtigungen auslösen.
// - suggest-category.ts (suggestCategory): leichtgewichtiger Einzel-Lookup für den interaktiven
//   Aufräum-Modus. Kein Side-Effect-Schema, kein Fuzzy-Fallback – absichtlich schlanker.
// Eine Zusammenführung würde die Trennlinie zwischen "Hintergrund-Pipeline" und "interaktivem Helfer"
// aufheben. Das gemeinsame Kern-Matching (conditionMatches/evaluateConditionGroups/findMatchingRule)
// ist bereits in suggest-category.ts zentralisiert und wird von pipeline.ts importiert (Zeilen oben).
...
async function findMerchantMatch(tx: PipelineTx, db: Database): Promise<MerchantMatchWithAlternatives | null> {
  ...
  const searchPurpose = tx.purpose ? normalizeCounterparty(tx.purpose) : "";
  ...
  >(
    `select m.id as merchant_id, m.default_category_id, ma.id as alias_id, ma.match_type, ma.match_field, ma.match_value, ma.priority
     ...
     order by 
       ...
       ma.priority asc,
       length(ma.match_value) desc,
       ma.id asc`
  );

  const matches: MerchantMatchResult[] = [];

  for (const a of aliases) {
    // Falls wir schon einen Top-Match haben, sammeln wir nur noch bis zu 2 Alternativen
    if (matches.length >= 3) break;

    let isMatch = false;
    let confidence = 0;
    let matchedBy: "merchant_iban" | "merchant_alias" = "merchant_alias";

    if (a.match_type === "iban") {
      if (recipientIban && a.match_value === recipientIban) {
        isMatch = true;
        confidence = 1.0;
        matchedBy = "merchant_iban";
      }
    } else {
      const fieldTargetsNorm = [];
      const fieldTargetsRaw = [];
      if (a.match_field === "counterparty" || a.match_field === "any") {
        if (searchCounterparty) fieldTargetsNorm.push(searchCounterparty);
        fieldTargetsRaw.push(rawCounterparty);
      }
      if (a.match_field === "purpose" || a.match_field === "any") {
        if (searchPurpose) fieldTargetsNorm.push(searchPurpose);
        if (rawPurpose) fieldTargetsRaw.push(rawPurpose);
      }

      if (a.match_type === "name_exact") {
        const normVal = normalizeCounterparty(a.match_value);
        if (fieldTargetsNorm.includes(normVal)) {
          isMatch = true;
          confidence = 0.95;
        }
        ...
        try {
          ...
          if (fieldTargetsRaw.some(t => reg.test(t))) {
            isMatch = true;
            confidence = 0.85;
          }
        } catch {
      ...
      } else if (a.match_type === "name_fuzzy") {
        const aliasNorm = normalizeCounterparty(a.match_value);
        let bestScore = 0;
        for (const target of fieldTargetsNorm) {
          const isSubstring = target.includes(aliasNorm) || aliasNorm.includes(target);
          const score = isSubstring ? 0.9 : calculateSimilarity(target, aliasNorm);
  ...
  if (matches.length > 0) {
    const distinct = dedupeByMerchant(matches);
    // Nur zurückgeben, wenn der beste Match ausreichend konfident ist (für Fuzzy >= 0.8 gefordert, 
    // aber wir prüfen das allgemein für den Top-Hit)
    if (distinct[0].confidence >= 0.8 || distinct[0].matched_by === "merchant_iban") {
