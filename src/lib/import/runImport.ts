...
const ParsedRowSchema = z.object({
  booking_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Ungültiges Buchungsdatum"),
  value_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Ungültiges Wertstellungsdatum").nullable(),
  counterparty: z.string().min(1, "Empfänger/Sender darf nicht leer sein"),
  purpose: z.string().nullable(),
  amount_cents: z.number().int("Betrag muss ein ganzzahliger Cents-Wert sein"),
  external_id: z.string().nullable(),
  fingerprint: z.string().min(1, "Fingerprint fehlt"),
  extra_fields_json: z.string().nullable(),
});
...
export function parseRows(input: RunImportInput): { parsed: ParsedRow[]; skipped: number; ignoredOtherAccount: number } {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const result = ParsedRowSchema.safeParse(rawParsed);
      if (!result.success) {
        const issues = result.error.issues.map(iss => `${iss.path.join('.')}: ${iss.message}`).join(', ');
        throw new Error(`Zeile ${i + 1}: Validierungsfehler - ${issues}`);
      }

      parsed.push(result.data);
    } catch (e) {
      if (e instanceof Error && e.message.startsWith("Zeile ")) {
        throw e; // Zod-Fehler direkt weiterwerfen (B1 requirement)
      }
      skipped += 1;
    }
  }
