...
// B3: Deterministische Alias-Priorität
describe("Deterministische Alias-Priorität (B3)", () => {
  it("name_exact gewinnt vor name_fuzzy auch wenn beide auf denselben Text treffen", async () => {
    const catExact = await createCategory({ name: "Exact-Kategorie", color: "#111", parent_id: null });
    const catFuzzy = await createCategory({ name: "Fuzzy-Kategorie", color: "#222", parent_id: null });

    const merchantExact = await createMerchant({ canonical_name: "prio-exact-merchant", display_name: "Prio Exact", default_category_id: catExact, is_builtin: 0 });
    const merchantFuzzy = await createMerchant({ canonical_name: "prio-fuzzy-merchant", display_name: "Prio Fuzzy", default_category_id: catFuzzy, is_builtin: 0 });

    // Fuzzy-Alias mit niedrigerem priority-Wert (= hoehere Prioritaet nach altem Denken)
    await addMerchantAlias({ merchant_id: merchantFuzzy, match_type: "name_fuzzy", match_field: "counterparty", match_value: "unique-prio-test-merchant", priority: 5 });
    // Exact-Alias mit hoeherem priority-Wert – soll trotzdem gewinnen, weil match_type praevaliert
    await addMerchantAlias({ merchant_id: merchantExact, match_type: "name_exact", match_field: "counterparty", match_value: "unique-prio-test-merchant", priority: 99 });

    const assetId = await createAsset({ name: "B3-Prio-Konto", kind: "account", account_type: "giro", owner_ids: [] });
    const txId = await insertRawTransaction({ asset_id: assetId, booking_date: "2024-06-01", counterparty: "unique-prio-test-merchant", amount_cents: -500 });

    await runPipelineForTransactions([txId]);
    const tx = await getTransactionRow(txId);

    // name_exact muss gewinnen – Exact-Merchant hat catExact
    expect(tx.merchant_id).toBe(merchantExact);
    expect(tx.category_id).toBe(catExact);
  });

  it("bei gleicher match_type und gleichem priority-Wert entscheidet match_field (counterparty vor purpose)", async () => {
    const catCp = await createCategory({ name: "Counterparty-Kat", color: "#333", parent_id: null });
    const catPurp = await createCategory({ name: "Purpose-Kat", color: "#444", parent_id: null });

    const merchantCp = await createMerchant({ canonical_name: "field-cp-merchant", display_name: "Field CP", default_category_id: catCp, is_builtin: 0 });
    const merchantPurp = await createMerchant({ canonical_name: "field-purp-merchant", display_name: "Field Purp", default_category_id: catPurp, is_builtin: 0 });

    await addMerchantAlias({ merchant_id: merchantPurp, match_type: "name_fuzzy", match_field: "purpose", match_value: "field-test-unique-keyword", priority: 10 });
    await addMerchantAlias({ merchant_id: merchantCp, match_type: "name_fuzzy", match_field: "counterparty", match_value: "field-test-unique-keyword", priority: 10 });

    const assetId = await createAsset({ name: "B3-Field-Konto", kind: "account", account_type: "giro", owner_ids: [] });
    const db = await getDb();
    const fingerprint = "2024-06-02|-600|field-test-unique-keyword";
    const res = await db.execute(
      "insert into transactions (asset_id, booking_date, counterparty, purpose, amount_cents, source, fingerprint) values ($1,$2,$3,$4,$5,'import',$6)",
      [assetId, "2024-06-02", "field-test-unique-keyword", "field-test-unique-keyword", -600, fingerprint],
    );
    const txId = res.lastInsertId as number;
    await runPipelineForTransactions([txId]);
    const tx = await getTransactionRow(txId);

    // counterparty-field-Alias muss vor purpose-field-Alias gewinnen
    expect(tx.merchant_id).toBe(merchantCp);
    expect(tx.category_id).toBe(catCp);
  });
});
